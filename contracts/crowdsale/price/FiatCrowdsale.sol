 ...
 */
contract FiatCrowdsale is Crowdsale, Ownable, usingOraclize {
	...
	/**
	 * @dev getter for token amount per wei amount
	 * @param _weiAmount amount in wei
	 * @return   token amount
	 */
	function getTokenAmount(uint256 _weiAmount) public view returns (uint256) {
		return _getTokenAmount(getFiatAmount(_weiAmount));
	}
	...
	*/
	function buyTokens(address _beneficiary) public payable {
		...
		uint256 tokens = _getTokenAmount(fiatAmount);
